...
::: {.dashboard-subcard}
#### Pacific Loggerhead Conservation Area Closures - <span id="closure-year">2024</span> Season {.dashboard-title}
...
::: {.dashboard-subcard}
#### El Niño Conditions and Coastal Water Temperatures - <span id="elnino-year">2024</span> Season {.dashboard-title}
...
::: {.dashboard-subcard}
#### TOTAL Time Series – <span id="bycatch-year">2024</span> {.dashboard-title}

<p id="bycatch-range">January – December 2024</p>
...
![](https://coastwatch.pfeg.noaa.gov/elnino/dash/indicator_2024.png){#bycatch-img fig-align="center"}
...
::: {.dashboard-subcard}
#### Sea Surface Temperature <span id="sst-year">2024</span> {.dashboard-title}

:::{.sst-dash-slider}
[‹ Previous]{.prev}
[Next ›]{.next}

![](https://coastwatch.pfeg.noaa.gov/elnino/dash/sst_20200316.png){#sst-dash-image}
:::
...
::: {.dashboard-subcard}
#### Sea Surface Temperature Anomaly <span id="anom-year">2024</span> {.dashboard-title}

:::{.anom-dash-slider}
[‹ Previous]{.prev}
[Next ›]{.next}

![](https://coastwatch.pfeg.noaa.gov/elnino/dash/anom_20200316.png){#anom-dash-image}
:::
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function getSSTImages(year) {
    ...
    return months.map(m => ({
      url: `https://coastwatch.pfeg.noaa.gov/elnino/dash/sst_${year}${m}16.png`,
      month: monthsMap[m]
    }));
  }
  function getAnomImages(year) {
    const months = ["03","04","05","06","07","08"];
    return months.map(m => ({
      url: `https://coastwatch.pfeg.noaa.gov/elnino/dash/anom_${year}${m}16.png`,
      month: monthsMap[m]
    }));
  }
  // === Load ENSO/SST CSV ===
  Papa.parse("environmental_conditions.csv", {
    download: true,
    header: true,
  ...
  // === Load Closure CSV ===
  Papa.parse("closure_history.csv", {
    download: true,
    header: true,
    complete: function(results) {
  ...
  function renderTable(year, unit) {
    const rows = allData.filter(row => row.year == year);
    tableBody.innerHTML = "";
  ...
  function updateBycatch(year) {
    ...
    if (bycatchImg)
      bycatchImg.src = `https://coastwatch.pfeg.noaa.gov/elnino/dash/indicator_${year}.png`;
  }
  // === SST & Anomaly Cards ===
  function updateSST() {
