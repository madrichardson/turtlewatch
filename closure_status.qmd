...
<div class="status-banner success">
  <span class="status-text">Loading closure status...</span>
...
// === Load Closure Status from Itca_closure.csv ===
fetch("data/resources/ltca_closure.csv")
  ...
  .then(text => {
    ...
    const parsed = rows
      ...
      .sort((a, b) => b.end - a.end); // Sort by most recent END date
    ...
    if (descEl) {
      ...
      if (now < recent.start) {
        descEl.textContent = `NMFS will prohibit fishing with drift gillnets (DGN) in the Pacific Loggerhead Conservation Area from ${startStr} to ${endStr}.`;
      } else if (now >= recent.start && now <= recent.end) {
        descEl.textContent = `NMFS is prohibiting fishing with drift gillnets (DGN) in the Pacific Loggerhead Conservation Area. The closure began on ${startStr} and remains in effect until ${endStr}.`;
      } else {
        descEl.textContent = `NMFS has reopened the Pacific Loggerhead Conservation Area (LCA) to fishing with drift gillnets (DGN). The LCA had been closed from ${startStr} to ${endStr}.`;
      }
    }
  })
