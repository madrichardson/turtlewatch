...
<div id="heat-regions">
  <table id="mhw-table" class="table table-condensed table-striped" style="display:none;">
    <thead>
      <tr><th>Region</th><th>Summary</th></tr>
    </thead>
    <tbody></tbody>
  </table>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // === Load Marine Heatwave JSON (regions only) ===
fetch("data/json/heatwave.json")
  ...
  .then(data => {
    ...
    const table = document.getElementById("mhw-table");
    const tbody = table ? table.querySelector("tbody") : null;
    if (!table || !tbody) return;

    const npText = (data.north_pacific || "").trim();
    const tpText = (data.tropical_pacific || "").trim();

    // Clear existing rows
    tbody.innerHTML = "";

    // Add rows only for the two regions we care about
    if (npText) {
      const tr = document.createElement("tr");
      const c1 = document.createElement("td"); c1.textContent = "North Pacific";
      const c2 = document.createElement("td"); c2.textContent = npText;
      tr.appendChild(c1); tr.appendChild(c2);
      tbody.appendChild(tr);
    }
    if (tpText) {
      const tr = document.createElement("tr");
      const c1 = document.createElement("td"); c1.textContent = "Tropical Pacific";
      const c2 = document.createElement("td"); c2.textContent = tpText;
      tr.appendChild(c1); tr.appendChild(c2);
      tbody.appendChild(tr);
    }

    // If neither region is present, show a single “Not available” row
    if (!npText && !tpText) {
      const tr = document.createElement("tr");
      const c1 = document.createElement("td"); c1.textContent = "North Pacific";
      const c2 = document.createElement("td"); c2.textContent = "Not available";
      tr.appendChild(c1); tr.appendChild(c2);
      tbody.appendChild(tr);

      const tr2 = document.createElement("tr");
      const d1 = document.createElement("td"); d1.textContent = "Tropical Pacific";
      const d2 = document.createElement("td"); d2.textContent = "Not available";
      tr2.appendChild(d1); tr2.appendChild(d2);
      tbody.appendChild(tr2);
    }

    // Show the table always; we’re not using heat_status anymore
    table.style.display = "";
