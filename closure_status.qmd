...
<!-- UPDATED SST IMAGES (local, dynamic like dashboard) -->
<script>
document.addEventListener("DOMContentLoaded", () => {
  const sstImgEl = document.getElementById("sst-image");
  ...
  let index = sstImages.length - 1;

  function updateImage() {
    sstImgEl.src = sstImages[index];
    prevBtn.style.opacity = index === 0 ? 0.5 : 1;
    nextBtn.style.opacity = index === sstImages.length - 1 ? 0.5 : 1;
  }
...
<!-- UPDATED SST ANOMALY IMAGES (local, dynamic like dashboard) -->
<script>
document.addEventListener("DOMContentLoaded", () => {
  const anomImgEl = document.getElementById("anom-image");
  const prevBtn = document.querySelector(".anom-slider .prev");
  const nextBtn = document.querySelector(".anom-slider .next");

  const year = new Date().getFullYear();
  const months = ["03","04","05","06","07","08"];
  const anomImages = months.map(m => `data/images/anom_${year}${m}16.png`);

  let index = anomImages.length - 1;

  function updateAnomImage() {
    anomImgEl.src = anomImages[index];
    prevBtn.style.opacity = index === 0 ? 0.5 : 1;
    nextBtn.style.opacity = index === anomImages.length - 1 ? 0.5 : 1;
  }
