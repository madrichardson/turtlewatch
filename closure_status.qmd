...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("data/json/heatwave.json")
    ...
    .then(data => {
      ...
      const hasRegion = Boolean(npText || tpText);

      if (hasRegion && table && tbody) {
        // Clear any previous rows
        tbody.innerHTML = "";

        if (npText) {
          const tr = document.createElement("tr");
          const r1 = document.createElement("td"); r1.textContent = "North Pacific";
          const r2 = document.createElement("td"); r2.textContent = npText;
          tr.appendChild(r1); tr.appendChild(r2);
          tbody.appendChild(tr);
        }

        if (tpText) {
          const tr = document.createElement("tr");
          const r1 = document.createElement("td"); r1.textContent = "Tropical Pacific";
          const r2 = document.createElement("td"); r2.textContent = tpText;
          tr.appendChild(r1); tr.appendChild(r2);
          tbody.appendChild(tr);
        }

        // Show table, hide combined fallback text
        table.style.display = "";
        if (hwStatus) hwStatus.style.display = "none";
      } else {
        // Fallback to combined text
        if (hwStatus) hwStatus.textContent = (data.heat_status || "Not available").trim();
