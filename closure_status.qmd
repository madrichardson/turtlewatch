...
document.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("data/resources/Itca_closure.csv")
    ...
    .then(text => {
      ...
      // Parse rows into date objects and sort by most recent start date
      const parsed = rows
        .map(r => ({
      ...
      if (descEl) {
        if (now < recent.start) {
          descEl.textContent = `NMFS will prohibit fishing with drift gillnets (DGN) in the Pacific Loggerhead Conservation Area from ${recent.start.toLocaleDateString()} to ${recent.end.toLocaleDateString()}.`;
        } else if (now >= recent.start && now <= recent.end) {
          descEl.textContent = `NMFS is prohibiting fishing with drift gillnets (DGN) in the Pacific Loggerhead Conservation Area. The closure began on ${recent.start.toLocaleDateString()} and remains in effect until ${recent.end.toLocaleDateString()}.`;
        } else {
          descEl.textContent = `NMFS has reopened the Pacific Loggerhead Conservation Area (LCA) to fishing with drift gillnets (DGN). The LCA had been closed from ${recent.start.toLocaleDateString()} to ${recent.end.toLocaleDateString()}.`;
        }
      }
    })
